import jiangLoading from './jiang-loading'
import Vue from 'vue'
...
const loading = {
  ...
  install(Vue, options) {
     ...
     *使用基础 Vue.extend 构造器，创建一个“子类” (Loading)。参数是一个包含组件选项的对象(jiangLoading)。
     *然后 创建一个 Loading 的实例 Profile 挂载到一个HTMLElement实例上
     */
    const Loading = Vue.extend(jiangLoading)
    ...
    const jiangLoadingMethod = {
      open(text) {
        Profile.show = true
        if (text) Profile.text = text
      },
      hide() {
        Profile.show = false
      }
    ...
    Vue.prototype.$jiangLoading = jiangLoadingMethod
  }
}

Vue.use(loading)

export default jiangLoading